...
: debug_(debug)
{
  ...
  // OpenVINO disabled due to GLIBC version incompatibility
...
std::list<Armor> YOLOV5::detect(const cv::Mat & raw_img, int frame_count)
{
  ...
  // Return empty list since OpenVINO is disabled
  tools::logger()->warn("OpenVINO disabled - returning empty detection results");
  return std::list<Armor>();
}
